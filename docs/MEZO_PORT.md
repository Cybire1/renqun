...
| A dropped transaction blocks every later one from the same account | `no receipt for 0x… after 120s`, then `invalid nonce; got 5819, expected 5818` on the settles behind it; five rounds voided (refunded) on 2026-09-17 | settles are sent at the chain's own next nonce and at a higher gas price, so they replace a stuck transaction instead of queuing behind it; every keeper transaction pays a multiple of the base gas price (a few hundred wei), and a missing receipt is given 45-60 s, not 120 |
...
| Front page | `web/components/Landing.tsx` | live ticker strip, a hero carrying the round running now, live pool and round numbers, three steps, Just ask, Earn, what a reader can verify, and an FAQ. Trading lives at `/markets` |
...
| Markets (`/markets`) | `web/components/Markets.tsx`, `Chart.tsx` | 5 min / 1 hour, the round's question, live oracle price, SVG chart green above the Up line and grey below, progress, just-closed result, last rounds, up next |
