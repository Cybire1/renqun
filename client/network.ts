// Mezo network config for Renqun's web client (the app keeps a twin in mobile/lib/mezo/network.ts).
//
//   NEXT_PUBLIC_MEZO_NETWORK=mainnet   → Mezo mainnet (chain 31612); default testnet (31611)
//   NEXT_PUBLIC_MEZO_RPC_URL, NEXT_PUBLIC_MEZO_PREDICT, NEXT_PUBLIC_MEZO_DEPLOY_BLOCK,
//   NEXT_PUBLIC_MEZO_DRIP_URL          optional overrides
//
...
// validators print every block. The venue is YosukuPredict (contracts/).
//
...
export const MEZO_NETWORK: MezoNetwork = process.env.NEXT_PUBLIC_MEZO_NETWORK === 'mainnet' ? 'mainnet' : 'testnet';
...
export interface MezoNetworkConfig {
  ...
  dexRouter: Address;
  dexFactory: Address;
  /** Starter drip (services/drip): gas for every player, plus test MUSD on testnet. */
  dripUrl: string;
}
...
const TESTNET: MezoNetworkConfig = {
  ...
  // Seen 2026-09-17: some clients could reach every Mezo host except rpc.test.mezo.org while dRPC
  // answered, so a second endpoint is not optional.
  ...
  predict: (process.env.NEXT_PUBLIC_MEZO_PREDICT as Address) || '0x85c9A910143A4814346132d93F221DE3FCF6536a',
  deployBlock: BigInt(process.env.NEXT_PUBLIC_MEZO_DEPLOY_BLOCK || 15_572_175),
  blockSeconds: 4,
  ...
  // In development the drip runs on this machine (services/drip).
  dripUrl: process.env.NEXT_PUBLIC_MEZO_DRIP_URL || (process.env.NODE_ENV === 'development' ? 'http://localhost:8787' : ''),
};
...
const MAINNET: MezoNetworkConfig = {
  ...
  predict: (process.env.NEXT_PUBLIC_MEZO_PREDICT as Address) || '',
  deployBlock: BigInt(process.env.NEXT_PUBLIC_MEZO_DEPLOY_BLOCK || 0),
  blockSeconds: 4,
  ...
  dripUrl: process.env.NEXT_PUBLIC_MEZO_DRIP_URL || '',
};
