// Getting money into a player's wallet: the starter drip and a prefilled MetaMask transfer.
//
//   • starter drip (services/drip): gas for every player; 20 test MUSD on testnet
//   • MetaMask: a prefilled MUSD transfer to this wallet (EIP-681 link)
//   • Mezo's DEX: BTC → MUSD, in venue.ts
...
export async function testMusdClaimed(address: Address): Promise<boolean> {
  return (await store.getItem(claimedKey(address)).catch(() => null)) === '1';
}

async function markTestMusdClaimed(address: Address): Promise<void> {
  await store.setItem(claimedKey(address), '1').catch(() => {});
}
